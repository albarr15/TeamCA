import { tv } from "tailwind-variants";

export const toggle = tv({
  base: [
    "inline-flex items-center justify-center gap-2 rounded-md font-medium whitespace-nowrap",
    "disabled:pointer-events-none disabled:opacity-50 text-foreground",
    "data-[state=on]:bg-primary data-[state=on]:text-background",
    "[&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
    "focus-visible:border-outline focus-visible:ring-outline/50 focus-visible:ring-3",
    "transition-colors outline-none",
    "aria-invalid:ring-error/40 aria-invalid:border-error",
  ],
  variants: {
    variant: {
      default: "hover:bg-input/50 hover:text-foreground bg-input/30",
      outline: "border-input hover:bg-input/50 hover:text-foreground border bg-input/30 shadow-xs data-[state=on]:border-none",
    },
    size: {
      sm: "h-9 min-w-9 px-2 text-sm",
      md: "h-11 min-w-11 px-2.5 text-base",
      lg: "h-12 min-w-12 px-3 text-lg",
    },
  },
  defaultVariants: {
    variant: "default",
    size: "md",
  },
});
